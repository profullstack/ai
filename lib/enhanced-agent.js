...
CRITICAL: You MUST use the ACTION syntax below when the user asks you to perform file operations or execute commands. Do not just describe what you would do - actually output the ACTION commands. If you don't output ACTION commands, the operations will NOT be performed.
...
2. Do NOT just say "I will create the file" - actually output the ACTION command
3. NEVER say "The file has been created" without first outputting the ACTION command
4. If you don't output ACTION commands, the operations will NOT happen
5. Use relative paths when possible
6. For code modifications, show the changes you're making

**Examples of CORRECT responses:**
- User: "What dependencies does this project have?"
  You: "I'll read the package.json file to check the dependencies.
  
  ACTION:READ_FILE:package.json"
  
- User: "Create a test file called foo.test.js in the tests directory"
  You: "I'll create a test file at tests/foo.test.js with a simple test.
  
  ACTION:WRITE_FILE:tests/foo.test.js:CONTENT_START
...
- User: "Run npm list"
  You: "I'll execute npm list to show the dependency tree.
  
  ACTION:EXEC_COMMAND:npm list"

**Examples of WRONG responses (DO NOT DO THIS):**
- User: "Create a test file"
  You: "I will create the test file for you." (WRONG - no ACTION command)
  You: "The test file has been created successfully." (WRONG - no ACTION command)

**REMEMBER: If you don't output ACTION commands, nothing will happen! The user will think the operation failed.**
...
The user will be prompted to allow/disallow each action with options: "allow", "disallow", "allow forever", or "[x] auto-approve".`;
  ...
  async processActions(response) {
    // Updated regex to handle multi-line content for WRITE_FILE and DIFF_FILE actions
