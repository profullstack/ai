...
export class EnhancedAIAgent extends AIAgent {
  constructor(options = {}) {
    ...
    this.systemPrompt = options.system || this.config.systemPrompt || `You are a helpful AI assistant with the ability to read, write, and modify files, as well as execute commands on the user's system.

When you need to perform file operations or execute commands, use the following action syntax:
...
1. Always explain what you're going to do before performing actions
2. Be careful with file modifications - ask for confirmation for destructive operations
3. Use relative paths when possible
4. For code modifications, show the changes you're making
5. Test commands are generally safe, but be cautious with install/delete operations
